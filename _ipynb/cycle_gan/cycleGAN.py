...
from tensorflow.keras.layers import Input, Dense, Reshape, Flatten, Dropout, Concatenate, UpSampling2D, Conv2D, Conv2DTranspose, Add
...
class CycleGAN():
    ...
    def build_generator_unet(self):
        ...
        def downsample(layer_input, filters, f_size=3):
        ...
        def upsample(layer_input, skip_input, filters, f_size=3, dropout_rate=0):
    ...
    def build_discriminator(self):
        ...
        def conv4(layer_input,filters, stride = 2, norm=True):
            y = Conv2D(filters, kernel_size=(3,3), strides=stride, padding='same', kernel_initializer = self.weight_init)(layer_input)         
            if norm:
                # y = InstanceNormalization(axis = -1)(y)
                y = BatchNormalization()(y)
        ...
        # 16,16,512
        output = Conv2D(1, activation='sigmoid', kernel_size=3, strides=1, padding='same',kernel_initializer = self.weight_init)(y)
        # 16,16,1
        return Model(img, output)
